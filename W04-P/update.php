<?php
  ...
  if (isset($_GET['id'])) {
      ...
      $query = "SELECT * FROM film WHERE id={$filtered_id}";
      $result = mysqli_query($link, $query);
...
<body>
  <h1><a href="index.php">Favorite 90s High-Teen Film</a></h1>
  <ul> <?= $list ?> </ul>
  <form action="process_update.php" method="post">
    <input type="hidden" name="id" value="<?=$filtered_id ?>">
    <p><input type="text" name="title" placeholder="title" value="<?=$article['title'] ?>"></p>
    <p><textarea name="mainactors" placeholder="mainactors"><?=$article['mainactors'] ?></textarea></p>
